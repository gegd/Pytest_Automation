...
class TestQueryInventory:
    ...
    def test_queryByItemBatchNo(self,purchasereceive_fix,_sm,_ft,param,_aid,_uid,_tenantid,_domid,_mt,expect):
        """采购订单收货完成后，根据采购订单号查询是否入批次库存"""
        log.info("采购订单收货完成后，根据采购订单号查询是否入批次库存")
        req_data = Inventory().get_req_data(_sm,_ft,param,_aid,_uid,_tenantid,_domid,_mt)
        new_data = json.loads(req_data['param'])
        new_data['purchaseOrderNo'] = purchasereceive_fix
        req_data['param'] = json.dumps(new_data)
        res = Request().get_request(Config().url_test,req_data)
