...
from common.common_function import login,CompanyBiz,ContactCompany,PurchaseAgreement,PurchaseOrder,Goods,WareHouse,PurchaseReceive
...
def ourCompany_fix():
    ...
    companyPageResultList = CompanyBiz().query_company()
    #获取查询列表条数
    if len(companyPageResultList) < 1:
        log.info("开始新增本方公司...")
        CompanyBiz().add_company()
...
def contactCompany_fix():
    ...
    companyPageResultList = ContactCompany().query_company()
    log.info("新增采购订单前先查询是否存在往来单位，如不存在则新增")
    if len(companyPageResultList) < 1:
        log.info("开始新增往来单位...")
        ContactCompany().add_company()
    return companyPageResultList
...
def purchaseorderadd_fix(addgoods_fix,createwarehouse_fix,pucharseAgreement_fix):
    ...
    log.info("采购订单新增完成，采购订单协议号：%s"%purchaseorderadd_fix)
    return res
